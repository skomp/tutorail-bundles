...
on `eth0`, and then discover that forwarding alone still does not work: the reply never comes
...
that can already reach the board but nothing beyond it. `eth0` plugged into your upstream
network and holding an address on it — a dynamic upstream address is fine and expected
(`#interface-roles`, `#address-plan`). You can read links, addresses and the routing table by
hand (lesson `01-reading-the-network`), which is how you will confirm what forwarding does. The
`tcpdump` package on the board; install it if it is absent (`apt install tcpdump`).
...
- Trace, using the routing table, how a packet from a `192.168.4.x` client to an internet
  address is matched against the default route and sent out `eth0`
- Use `tcpdump` on `eth0` to observe that a client's forwarded packet leaves the board carrying
...
via `eth0` — and the packet is forwarded out `eth0` toward the internet. Forwarding is working.
The packet really leaves.

And here is the wall. That packet left `eth0` still carrying its original source address,
...
The fix is to rewrite it: as each client packet leaves `eth0`, replace its private source with
...
and the **default route** sends an internet-bound packet out `eth0`; the anatomy of the client
packet — source `192.168.4.x`, destination on the internet — as it arrives on `wlan0` and as it
leaves on `eth0` unchanged; **private (RFC 1918) address ranges** and what "not routable on the
...
- Do **not** add NAT, masquerading, or any nftables rule in this lesson. A client whose packets
  leave `eth0` but get no reply is the correct end state here — that is the observation the
...
client's actual address) — and, in another session, `tcpdump -ni eth0 host 192.168.4.50`.
Re-run the client's ping. You will see the request arrive on `wlan0` and **nothing** appear on
`eth0`: the packet is not being forwarded. That is the endpoint behaviour you are about to
change.
...
(`ip route`) and find the **default route** via `eth0`; that is the route your client's
internet-bound packets will now match.

Re-run the two `tcpdump` captures and the client's ping. This time the story changes on `eth0`:
you will see the client's ICMP echo request **leave `eth0`**, and — read the addresses in the
...
observe it. State it in your own words: the packet left `eth0` with a private `192.168.4.x`
...
board's routable `eth0` address on the way out and remember the mapping to reverse it on the
...
- With `tcpdump` on `eth0`, you have **seen** a client's forwarded packet leave `eth0` carrying
  its private `192.168.4.x` source address — not merely inferred it. You can point to the
  request on `eth0` and to the absence of any reply.
...
- That forwarding **alone is not enough** for clients to reach the internet: their packets
  leave `eth0` with a private `192.168.4.x` source and no reply can return.
