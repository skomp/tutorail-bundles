...
flow with its private source on `wlan0` and rewritten to `eth0`'s address on
`eth0`, so the NAT translation from lesson 06 stops being a rule you trust and
becomes a thing you observed.
...
`192.168.4.x`; the same connection leaving `eth0` carries `eth0`'s upstream
address instead. Two captures, one flow, the translation visible in both. This is
an observation lesson — you change nothing about how the appliance runs.
...
- A working NATed access point: lesson 06 (`06-nat-with-nftables`) complete.
  Client traffic from `192.168.4.0/24` is source-NATed (`masquerade`) out `eth0`,
  and a Wi-Fi client can reach the internet through the box.
- The interface roles from the design: `wlan0` faces the clients, `eth0` is the
  upstream. All captures in this lesson name those two interfaces.
...
- Capture one client's flow on `wlan0` (private source `192.168.4.x`) and the same
  flow on `eth0` (source rewritten to `eth0`'s address), and point to the single
  translation that connects the two.
...
routed, and on the way out `eth0` the `masquerade` rule rewrites that source to
`eth0`'s own upstream address (this is exactly the postrouting rewrite from lesson
06). So:
...
- **Capture on `eth0`** and you see the *post-NAT* view of the same packet: source
  is now `eth0`'s address, destination unchanged. The client's private address has
  disappeared from the header.

The same reasoning runs in reverse for replies: on `eth0` the reply is addressed
to `eth0`'s address; by the time it leaves `wlan0` conntrack has un-translated it
back to `192.168.4.x`. Watching both interfaces is watching both ends of the
...
`eth0`'s address on `wlan0`, where it never appears. Read the two captures
...
- The interface choice selects the side of NAT: `wlan0` shows the pre-NAT private
  source, `eth0` shows the same flow with the source rewritten to `eth0`'s address.
- The reverse direction: replies arrive on `eth0` for `eth0`'s address and leave
  `wlan0` un-translated back to the client — the mirror of the outbound rewrite.
...
- The learner must capture the *same* flow on both `wlan0` and `eth0` — one
  capture on one interface does not show the translation. Both sides are required
  to see it.
...
5. Read the current `eth0` address (`ip -4 addr show eth0`) so you know what to
   look for on the other side.
6. Capture the post-NAT view of the *same* flow: `tcpdump -n -i eth0` filtered to
   the same destination or port (for example `host <far-host> and port 443`, or
   `icmp`). Confirm the source is now `eth0`'s address and the private
   `192.168.4.x` no longer appears.
7. Put the two side by side and name the translation: same destination, same
   protocol/port, source `192.168.4.x` on `wlan0` becomes `eth0`'s address on
   `eth0`. Have the learner say aloud which capture is pre-NAT and which is post,
   and why.
...
9. Instructive probes: capture on the wrong interface (look for `eth0`'s address
   on `wlan0`) and see it never appear; run an unfiltered `tcpdump -i eth0` briefly
   to feel the noise, then re-apply the filter and watch the signal return.
...
- The learner captures the *same* flow on `eth0` and shows the source is now
  `eth0`'s upstream address, with `192.168.4.x` absent from the header (the
  post-NAT view).
...
- The learner has observed the NAT translation on the wire: the same client flow
  with a private `192.168.4.x` source on `wlan0` and `eth0`'s address on `eth0`,
  and has connected this to the `conntrack`/`masquerade` behaviour from lesson 06.
...
- Tighten to a single client and single conversation: combine `host <client-ip>
  ...
  `eth0`, to watch exactly one flow cross the box with nothing else in the way.
- Watch a DNS exchange: capture `port 53` on `wlan0` to see a client's query to
  the box's resolver, then on `eth0` to see the box's own upstream lookup — a
