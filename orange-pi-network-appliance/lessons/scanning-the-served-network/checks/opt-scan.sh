...
found="$(bssh 'if command -v arp-scan >/dev/null; then sudo arp-scan --interface=wlan0 192.168.4.0/24 2>/dev/null | grep -c "192\.168\.4\."; else sudo nmap -sn 192.168.4.0/24 2>/dev/null | grep -c "Nmap scan report"; fi' || echo 0)"
