...
| 02 | `the-lifeline` | an out-of-band way back in that does not depend on IP | Bluetooth SPP/RFCOMM; getty over a serial line; udev device activation and rule authoring | `serial-getty@rfcomm0` times out because a `/dev` node is not a systemd `.device` unit until a udev rule tags it (diagnosed, then fixed with a rule the learner writes); forgetting to advertise SPP; assuming the lifeline needs an address | learner drops Ethernet and still gets a shell over Bluetooth | `#recovery-invariant` | `lifeline-up` |
